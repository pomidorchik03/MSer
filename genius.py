...
genius = lyricsgenius.Genius(GENIUS_ACCESS_TOKEN, user_agent=session, timeout=15, retries=3)
...
def get_lyrics_safe(song_name: str, artist_name: str):
    ...
    try:
        ...
        for hit in results["hits"]:
            ...
            if song_data["title"].lower()[:len(song_name)] == song_name.lower() and \
               song_data["primary_artist"]["name"].lower() == artist_name.lower():
        ...
        return get_lyrics_direct(song_name, artist_name) or get_lyrics_ovh(song_name, artist_name)
